...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <div class="glass-dark rounded-2xl p-8 border border-slate-200/80 dark:border-white/10 shadow-sm">
        ...
        <form action="{{ route('admin.products.update', $product) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div class="grid grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <select name="is_active" class="input-field w-full rounded-xl py-3 px-4 text-sm appearance-none">
                        <option value="1" class="bg-slate-900" {{ old('is_active', $product->is_active) ? 'selected' : '' }}>Active</option>
                        <option value="0" class="bg-slate-900" {{ !old('is_active', $product->is_active) ? 'selected' : '' }}>Inactive</option>
                    </select>
                </div>
            </div>
            ...
            <div class="grid grid-cols-2 gap-5">
                <div>
                    <label class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Upload New File <span class="text-slate-600">(browse)</span></label>
